...
class AnaliseService:
    ...
    def alertas(self, empresa_id: int, usuario_id: int) -> list[AlertaResposta]:
        ...
        for cr in self.repository.contasReceberVencidas(empresa_id, usuario_id):
            resultado.append(AlertaResposta(
                tipo=0,
    ...
    def calendario(self, empresa_id: int, usuario_id: int, mes: int | None, ano: int | None) -> DadosCalendarioResposta:
        ...
        pagar   = self.repository.contasPagarPorMes(empresa_id, usuario_id, m, a)
        receber = self.repository.contasReceberPorMes(empresa_id, usuario_id, m, a)
    ...
    def _chat_pagar(self, empresa_id: int, usuario_id: int) -> str:
        contas = self.repository.contasPagarPendentes(empresa_id, usuario_id)
        if not contas:
    ...
    def _chat_problemas(self, empresa_id: int, usuario_id: int) -> str:
        pagar   = self.repository.contasPagarVencidas(empresa_id, usuario_id)
        receber = self.repository.contasReceberVencidas(empresa_id, usuario_id)
    ...
    def _chat_receber(self, empresa_id: int, usuario_id: int) -> str:
        contas = self.repository.contasReceberPendentes(empresa_id, usuario_id)
        if not contas:
