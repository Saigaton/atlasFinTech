...
from app.configuracoes.dependencies import obterAuthService
...
from app.utilitarios.email_utilitario import corpoEmailParaRecuperarSenha, corpoEmailVerificacao, dispararEmailComTentativas
...
async def registro(body: RequisicaoRegistroUsuario, service: AuthService = Depends(obterAuthService)):
...
async def login(body: RequisicaoLoginUsuario, service: AuthService = Depends(obterAuthService)):
...
async def verificarEmail(token: str, service: AuthService = Depends(obterAuthService)):
...
async def logout(body: RequisicaoTokenAtualizacao, service: AuthService = Depends(obterAuthService), _: dict = Depends(obterUsuarioAtual)):
...
async def tokenAtualizacao(body: RequisicaoTokenAtualizacao, service: AuthService = Depends(obterAuthService)):
...
async def reenviarVerificacaoEmail(request: Request, backgroundTasks: BackgroundTasks, body: RequisicaoEmail, service: AuthService = Depends(obterAuthService)):
...
async def esqueceuSenha(request: Request, backgroundTasks: BackgroundTasks, body: RequisicaoEmail, service: AuthService = Depends(obterAuthService)):
...
async def redefinirSenha(body: RequisicaoRedefinirSenha, service: AuthService = Depends(obterAuthService)):
...
async def atualizarPerfil(body: RequisicaoAtualizarPerfil, service: AuthService = Depends(obterAuthService), usuarioAtual: dict = Depends(obterUsuarioAtual)):
...
async def loginGoogle(body: RequisicaoLoginGoogle, service: AuthService = Depends(obterAuthService)):
...
async def definirSenha(body: RequisicaoDefinirSenha, service: AuthService = Depends(obterAuthService), usuarioAtual: dict = Depends(obterUsuarioAtual)):
...
async def trocarSenha(body: RequisicaoTrocarSenha, service: AuthService = Depends(obterAuthService), usuarioAtual: dict = Depends(obterUsuarioAtual)):
